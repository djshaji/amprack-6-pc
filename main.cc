...
void activate (GApplication * app, void * v) {
	...
	if ( std::filesystem::exists ("themes/TubeAmp/style.css")) {
		gtk_css_provider_load_from_path(cssProvider, std::string ("themes/TubeAmp/style.css").c_str ());
        logd ("[theme] using themes/TubeAmp/style.css\n");
    }
	else if ( std::filesystem::exists ("/usr/share/amprack/themes/TubeAmp/style.css")) {
		gtk_css_provider_load_from_path(cssProvider, std::string ("/usr/share/amprack/themes/TubeAmp/style.css").c_str ());
		logd ("[theme] using /usr/share/amprack/themes/TubeAmp/style.css\n");
	}
    else {
        gtk_css_provider_load_from_path(cssProvider, "style.css");
        logd ("[theme] using default style.css\n");
    }
    ...
    if (std::filesystem::exists (user_css)) {
        gtk_css_provider_load_from_path(cssProvider2, user_css.c_str());
        logd ("[theme] using user style.css\n");
    } 
