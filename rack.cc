...
Rack::Rack () {
    ...
    std::string version = std::string ("<big><b>Tone Raptor SJ build ").append (std::to_string (VERSION)).append ("</b></big>");
    GtkLabel * title = (GtkLabel * )gtk_label_new (version.c_str ()) ;
    gtk_label_set_markup (title, version.c_str ());
    gtk_widget_set_visible ((GtkWidget *) title, true);
    gtk_widget_set_name ((GtkWidget *) title, "title");
